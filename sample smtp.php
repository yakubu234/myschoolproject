...
if ( isset( $_POST['contact'] ) ):
    ...
    $name = filter_var( $_POST['name'], FILTER_SANITIZE_STRING );// name from textbox
    $from_email = filter_var( $_POST['email'], FILTER_SANITIZE_EMAIL );//  email fromtextbox
    $number = filter_var( $_POST['number'], FILTER_SANITIZE_STRING );//number from textbox
    $message = filter_var( $_POST['message'], FILTER_SANITIZE_STRING );//message from textbox

    /* insert int database 
    $ins = "INSERT INTO contact VALUES (null,'$name',' $from_email','$number','$message',null)";
    $con = $conn->query($ins);
    if ($con) {
        echo "<script>
